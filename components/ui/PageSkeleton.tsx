...
export function PageSkeleton({ title, rows = 4, withStats = false }: PageSkeletonProps) {
  return (
    <ScreenContainer header={<Header title={title} />}>
        ...
        <div className="grid grid-cols-3 gap-2 mt-1 mb-4">
          {[0, 1, 2].map((i) => (
            <Card key={i} className="py-4 px-2">
              <div className="h-6 bg-(--border) rounded animate-pulse mx-auto w-12" />
              <div className="h-3 bg-(--border) rounded animate-pulse mt-2 mx-auto w-16" />
            </Card>
          ))}
        </div>
      )}
        ...
        {Array.from({ length: rows }).map((_, i) => (
          <Card key={i} className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="h-3 bg-(--border) rounded animate-pulse w-1/3 mt-2" />
            </div>
          </Card>
        ))}
      </div>
