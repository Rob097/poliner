...
export async function switchPollaio(pollaioId: string): Promise<ActionResult> {
  ...
  if (!member) {
    return { ok: false, error: "Non fai parte di questo pollaio." };
  }
  ...
  if (error) {
    return { ok: false, error: "Non sono riuscito a cambiare pollaio. Riprova!" };
  }
...
export async function creaPollaio(
  ...
  if (error || !data) {
    return { ok: false, error: "Ops, non sono riuscito a creare il pollaio. Riprova!" };
  }
...
export async function abbandonaPollaio(pollaioId: string): Promise<ActionResult> {
  ...
  if (error) {
    return { ok: false, error: "Non sono riuscito a rimuoverti dal pollaio. Riprova!" };
  }
...
export async function rimuoviMembro(input: {
  ...
  if (input.userId === user.id) {
    return { ok: false, error: "Per andartene tu, usa 'Lascia pollaio'." };
  }
  ...
  if (!mio || mio.ruolo !== "admin") {
    return { ok: false, error: "Solo gli admin possono rimuovere membri." };
  }
  ...
  if (target.ruolo === "admin") {
    ...
    if ((count ?? 0) <= 1) {
      return { ok: false, error: "Non puoi rimuovere l'ultimo admin del pollaio." };
    }
  }
  ...
  if (error) {
    return { ok: false, error: "Non sono riuscito a rimuovere il membro. Riprova!" };
  }
...
export async function cambiaRuoloMembro(input: {
  ...
  if (!mio || mio.ruolo !== "admin") {
    return { ok: false, error: "Solo gli admin possono cambiare i ruoli." };
  }
  ...
  if (input.nuovoRuolo === "guest") {
    ...
    if (target?.ruolo === "admin") {
      ...
      if ((count ?? 0) <= 1) {
        return { ok: false, error: "Non puoi degradare l'ultimo admin a guest." };
      }
    }
  }
  ...
  if (error) {
    return { ok: false, error: "Non sono riuscito ad aggiornare il ruolo. Riprova!" };
  }
